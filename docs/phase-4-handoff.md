...
- Replaced the static `/board` placeholder with a current-week board backed by PocketBase data.
- Added server-side board data loading for members, current-week fresh listens, expanded albums, and reactions.
...
- Added empty member slots, including a current-user "Draw your pick" CTA when the signed-in member has no current-week fresh pick.
- Added PocketBase realtime subscriptions for current-week fresh listens and reactions.
...
- `lib/board.ts`
  ...
  - Fetches all authenticated members and current-week `fresh` listens.
  - Selects the latest current-week fresh listen per member in the client card projection.
...
- `app/(club)/board/actions.ts`
  ...
  - Validates the target listen is a current-week fresh listen.
...
- The board displays the latest current-week fresh listen per member. The MVP has no weekly lockout, so a fast user could technically create multiple rated fresh listens in the same ISO week; only the newest one is shown as the headline board card.
...
- /board now renders real current-week board state from PocketBase via lib/board.ts.
...
- The board shows the latest current-week fresh listen per member.
